...
// IG-ONLY: if true - prod / if true - dev  CLIENT IDS, KEYS AND TOKENS FOR
// INSTAGRAM TWITTER AND SPOTIFY  CLIENT IDS, KEYS AND TOKENS FOR INSTAGRAM
// TWITTER AND SPOTIFY  CLIENT IDS, KEYS AND TOKENS FOR INSTAGRAM TWITTER AND
// SPOTIFY
...
if (ignitionSwitch) {
  ...
  instagramApiURL =
    "http://www.instagram.com/oauth/authorize?client_id=b23670e220f14f1c89c11f627c9f9" +
    "953&redirect_uri=http://www.losethequit.me/views/werkspayce.html&response_type=c" +
    "ode&scope=basic+public_content+follower_list+comments+relationships+likes";
  ...
  instagramApiURL =
    "http://www.instagram.com/oauth/authorize?client_id=d0f6230a40954cb2823768aa53910" +
    "a5e&redirect_uri=http://localhost:5000/views/werkspayce.html&response_type=code&" +
...
//  CLIENT IDS, KEYS AND TOKENS FOR INSTAGRAM TWITTER AND SPOTIFY  CLIENT IDS,
// KEYS AND TOKENS FOR INSTAGRAM TWITTER AND SPOTIFY  CLIENT IDS, KEYS AND
// TOKENS FOR INSTAGRAM TWITTER AND SPOTIFY
...
// THE HOMEPAGE THE HOMEPAGE THE HOMEPAGE
...
// THE HOMEPAGE THE HOMEPAGE THE HOMEPAGE EMCIEN STUFF EMCIEN STUFF EMCIEN STUFF
...
app.post("/emcien", function(req, res) {
  ...
  client.get(exampleBaseUrl, function(data, response) {
    ...
    // console.log(data); console.log(response); res.send(data.items); res.end();
  });
...
// EMCIEN STUFF EMCIEN STUFF EMCIEN STUFF THIS IS USED TO LOAD GENERIC SPOTIFY
...
app.get("/spotify", function(req, res) {
  ...
  var spotifyApiUrl =
    "https://api.spotify.com/v1/artists/1vCWHaC5f2uS3yhpwWbIA6/albums?album_type=SING" +
    "LE&offset=20&limit=20";
...
app.get("/views/werkspayce.html/spotify-login", function(req, res) {
  ...
  // 'http://www.losethequit.me/views/werkspayce.html/spotify-callback' IS THE
  // REDIRECT URL AND IS THE SUBSEQUENT METHOD BELOW THIS ONE
  ...
  if (state === null || state !== storedState) {
    ...
    request.post(authOptions, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        // IF EVERYTHING GOES WELL PROCEED TO AUTHENTICATE IG
...
app.post("/ig", function(req, res, next) {
  ...
  if (req.body.token.indexOf("http") != -1) {
    ...
    // if the access code IS NOT available STOP the process of making a call to the
    // instagram api
  } else {
    // if the access code IS available START the process of making a call to the
    // instagram api
    ...
    request(post_options, function(error, response, body) {
      ...
      if (response.statusCode != 200) {
        ...
        // &min_id=678453535718114828_919796408 console.log(igEndPoints.self_search);
...
// AFTER AUTHENTICATION - THIS METHOD PINGS IF FOR DATA SPECIFIED BY THE
// "REQUEST OPTION"
...
// init.next().value.then((thePromise) => {   console.warn(thePromise);   //
// console.warn(init.next());   return init.next().value; });
// init.next().value.then((thePromise) => {   console.warn(thePromise); })
...
function smartCode(generator) {
  ...
  // gen.value.then(function(d) {   if (!d.temperature) {
  // runPolling(generator);   } else {     console.log(d);   } });
}
...
// function* doWhile() {   let url = 'http://flonoware.herokuapp.com/homebrew'
// let response = yield fetch(url);   let stepTwo = yield getLocation;   let
// coordsData = yield fetchWrapper();   let stepOne = yield
// setUserName(randomiZer()); };
...
// let match = curry((what, str) => str.match(what)); let match = ((what) =>
// (str) => str.match(what));
